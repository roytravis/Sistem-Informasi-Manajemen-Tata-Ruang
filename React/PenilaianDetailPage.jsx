    ...
    <style>
        {`
            ...
            @media print {
                ...
                .printable-area fieldset { padding: 0.4rem !important; margin-top: 0.4rem !important; }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1 p-2 border rounded-md signature-block">
            ...
            <div>
                ...
                <div className="my-0.5 signature-image-container">
                    {signaturePath ? (
                        <img src={imageUrl} alt={`Tanda Tangan ${member.nama}`} className="mx-auto h-16 border rounded bg-white" />
                    ) : (
...
export default function PenilaianDetailPage() {
        ...
        const fetchDetail = async () => {
            ...
            try {
                ...
                if (penilaianData) {
                    ...
                    if (['Baru', 'Menunggu Penilaian'].includes(kasusData.status)) {
                        ...
                        // Data sudah ada & status bukan baru -> TERKUNCI (Read Only)
                        setIsReadOnly(true);
                    }
    ...
    // --- RENDER TOMBOL EDIT REQUEST ---
    const renderEditButton = () => {
        // Hanya tampilkan jika data sudah ada dan user memiliki hak akses (Koordinator/Admin/Ketua Tim)
        if (!initialPenilaianExists || !user || !['Admin', 'Koordinator Lapangan', 'Ketua Tim'].includes(user.role)) return null;
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-3">
                    ...
                    {isReadOnly && initialPenilaianExists && (
                        <div className="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 print:hidden no-print" role="alert">
                            ...
                            <p>Formulir ini terkunci. Klik tombol 'Ajukan Edit' untuk mengubah data.</p>
                        </div>
                    )}
                    ...
                    <fieldset className="border p-3 rounded-md">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1 mt-1 text-sm p-1">
                            ...
                            <p><strong className="font-medium text-gray-600 w-32 inline-block">Nomor Identitas</strong>: {kasus.pemegang?.nomor_identitas || '-'}</p>
                    ...
                    {!isReadOnly && (
                        <div className="flex justify-end pt-3 print:hidden no-print space-x-3">
                            ...
                            >
                                {submitLoading ? 'Menyimpan...' : (initialPenilaianExists ? 'Simpan Perubahan & Kunci' : 'Simpan Hasil Penilaian')}
                            </button>
                        </div>
                    )}
                </form>
